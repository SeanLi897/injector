...
volatile uint8_t Tx1_cplt_flag = 0;
volatile uint8_t uart1_idle_flag = 0;  // 空闲中断标志
volatile uint16_t uart1_received_len = 0;  // 接收到的数据长度

volatile uint8_t Tx2_cplt_flag = 0;
volatile uint8_t uart2_idle_flag = 0;  // 空闲中断标志
volatile uint16_t uart2_received_len = 0;  // 接收到的数据长度

volatile uint8_t Tx3_cplt_flag = 0;
volatile uint8_t uart3_idle_flag = 0;  // 空闲中断标志
volatile uint16_t uart3_received_len = 0;  // 接收到的数据长度

uint8_t rx1_buffer[RX_BUFFER_SIZE];
uint8_t rx2_buffer[RX_BUFFER_SIZE];
uint8_t rx3_buffer[RX_BUFFER_SIZE];
/* USER CODE END 0 */
